...
func (cs *ControllerServer) createHostPVCBackup(
	ctx context.Context,
	hotsPb *harvesterv1beta1.PVCBackup,
) (*harvesterv1beta1.PVCBackup, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCBackups(hotsPb.Namespace).Create(ctx, hotsPb, metav1.CreateOptions{})
}

func (cs *ControllerServer) getHostPVCBackup(ctx context.Context, ns, hostPb string) (*harvesterv1beta1.PVCBackup, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCBackups(ns).Get(ctx, hostPb, metav1.GetOptions{})
}

func (cs *ControllerServer) deleteHostPVCBackup(ctx context.Context, ns, name string) error {
	return cs.harvClient.HarvesterhciV1beta1().PVCBackups(ns).Delete(ctx, name, metav1.DeleteOptions{})
}

func (cs *ControllerServer) listHostPVCBackups(ctx context.Context,
	ns string,
	lo metav1.ListOptions,
) (*harvesterv1beta1.PVCBackupList, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCBackups(ns).List(ctx, lo)
}

func (cs *ControllerServer) createHostPVCRestore(
	ctx context.Context,
	hostPr *harvesterv1beta1.PVCRestore,
) (*harvesterv1beta1.PVCRestore, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCRestores(hostPr.Namespace).Create(ctx, hostPr, metav1.CreateOptions{})
}

func (cs *ControllerServer) getHostPVCRestore(ctx context.Context, ns, hostPr string) (*harvesterv1beta1.PVCRestore, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCRestores(ns).Get(ctx, hostPr, metav1.GetOptions{})
}

func (cs *ControllerServer) deleteHostPVCRestore(ctx context.Context, ns, name string) error {
	return cs.harvClient.HarvesterhciV1beta1().PVCRestores(ns).Delete(ctx, name, metav1.DeleteOptions{})
}

func (cs *ControllerServer) listHostPVCRestores(ctx context.Context,
	ns string,
	lo metav1.ListOptions,
) (*harvesterv1beta1.PVCRestoreList, error) {
	return cs.harvClient.HarvesterhciV1beta1().PVCRestores(ns).List(ctx, lo)
}
...
func (cs *ControllerServer) generateHostPVCRestore(
	ctx context.Context,
	name string,
	pb *harvesterv1beta1.PVCBackup,
) (*harvesterv1beta1.PVCRestore, error) {
	newPr := &harvesterv1beta1.PVCRestore{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: cs.namespace,
		},
		Spec: harvesterv1beta1.PVCRestoreSpec{
			Type: harvesterv1beta1.PVCRestoreLH,
			From: fmt.Sprintf("%s/%s", pb.Namespace, pb.Name),
		},
	}

	return cs.createHostPVCRestore(ctx, newPr)
}

func (cs *ControllerServer) getOrCreatePVCRestore(
	ctx context.Context,
	name string,
	pb *harvesterv1beta1.PVCBackup,
) (*harvesterv1beta1.PVCRestore, error) {
	existingPr, err := cs.getHostPVCRestore(ctx, cs.namespace, name)
	if err == nil {
		logrus.Infof("PVCRestore %s/%s already exists", existingPr.Namespace, existingPr.Name)
	...
	createdPr, err := cs.generateHostPVCRestore(ctx, name, pb)
	if err != nil {
		return nil, err
	}

	logrus.Infof("Created PVCRestore %s/%s from PVCBackup %s/%s", createdPr.Namespace, createdPr.Name, pb.Namespace, pb.Name)
	return createdPr, nil
}

func (cs *ControllerServer) buildHostPVCRestoreFromBackup(
	ctx context.Context,
	name string,
	pb *harvesterv1beta1.PVCBackup,
) (*harvesterv1beta1.PVCRestore, error) {
	pr, err := cs.getOrCreatePVCRestore(ctx, name, pb)
	if err != nil {
		return nil, err
	}

	if !cs.checkPVCRestoreReadiness(ctx, pr.Name) {
		return nil, status.Errorf(codes.Internal, "PVCRestore %s is not ready", pr.Name)
	}
...
func (cs *ControllerServer) buildHostPVCRestore(
	ctx context.Context,
	...
	isBackup, pbNs, pbName := parseSnapshotID(snapshotID)
	if !isBackup {
		return nil, status.Error(codes.InvalidArgument, "Invalid backup snapshot ID format")
	}

	// Get the source PVCBackup to verify it exists
	pb, err := cs.getHostPVCBackup(ctx, pbNs, pbName)
	if err != nil {
		return nil, err
	}

	pr, err := cs.buildHostPVCRestoreFromBackup(ctx, req.GetName(), pb)
	if err != nil {
		return nil, err
	}
	logrus.Infof("Created PVCRestore %s/%s for volume %s", pr.Namespace, pr.Name, req.GetName())

	// Extract volume size from PVCBackup's status.sourcespec
	var volSizeBytes int64
	if pb.Status.SourceSpec.Resources.Requests != nil {
		if storage := pb.Status.SourceSpec.Resources.Requests.Storage(); storage != nil {
...
func (cs *ControllerServer) deletePVCRestoreIfExists(ctx context.Context, volumeID string) (bool, error) {
	pr, err := cs.getHostPVCRestore(ctx, cs.namespace, volumeID)
	if errors.IsNotFound(err) {
		return false, nil
	}

	if err != nil {
		logrus.Warnf("Failed to get PVCRestore %s: %v, continuing with PVC deletion", volumeID, err)
		return false, err
	}

	logrus.Infof("Found PVCRestore %s/%s for volume %s, deleting it", pr.Namespace, pr.Name, volumeID)
	err = cs.deleteHostPVCRestore(ctx, cs.namespace, volumeID)
	if err != nil {
		return false, err
	}

	logrus.Infof("Successfully deleted PVCRestore %s/%s", cs.namespace, volumeID)
	return true, nil
...
func (cs *ControllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	// Check if there is a PVCRestore with the same name as the volume and delete it
...
// convertPVCBackupToCSI converts a PVCBackup to a CSI Snapshot
func (cs *ControllerServer) convertPVCBackupToCSI(pb *harvesterv1beta1.PVCBackup, vol string) (*csi.Snapshot, error) {
	if pb == nil {
		return nil, fmt.Errorf("PVCBackup cannot be nil")
	}

	if pb.Name == "" {
		return nil, fmt.Errorf("PVCBackup name cannot be empty")
	}
	...
	var creationTime *timestamppb.Timestamp
	if !pb.CreationTimestamp.IsZero() {
		creationTime = timestamppb.New(pb.CreationTimestamp.Time)
	}

	var sizeBytes int64
	if requests := pb.Status.SourceSpec.Resources.Requests; requests != nil {
		sizeBytes = requests.Storage().Value()
	}

	var readyToUse bool
	readyToUse = pb.Status.Success

	return &csi.Snapshot{
		SnapshotId:     formatBackupSnapshotID(pb.Namespace, pb.Name),
		SourceVolumeId: vol,
		SizeBytes:      sizeBytes,
		CreationTime:   creationTime,
		ReadyToUse:     readyToUse,
...
func (cs *ControllerServer) checkPVCBackupReadiness(ctx context.Context, name string) bool {
	pb, err := cs.getHostPVCBackup(ctx, cs.namespace, name)
	if err != nil {
		logrus.Infof("Error retrieving PVCBackup %s: %v", name, err)
		return false // continue waiting
	}

	if pb.Status.Success {
		logrus.Infof("PVCBackup %s is ready", name)
		return true // stop waiting, ready
	}
...
func (cs *ControllerServer) checkPVCRestoreReadiness(ctx context.Context, name string) bool {
	pr, err := cs.getHostPVCRestore(ctx, cs.namespace, name)
	if err != nil {
		logrus.Infof("Error retrieving PVCRestore %s: %v", name, err)
		return false // continue waiting
	}

	if pr.Status.Success {
		logrus.Infof("PVCRestore %s is ready", name)
		return true // stop waiting, ready
	}
...
func (cs *ControllerServer) prepareBackupSnapResponse(
	...
	if !cs.checkPVCBackupReadiness(ctx, name) {
		return nil, status.Errorf(codes.Internal, "PVCBackup %s is not ready", name)
	}

	readyPb, err := cs.getHostPVCBackup(ctx, cs.namespace, name)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to get PVCBackup %s: %v", name, err)
	}

	csiSnapshot, err := cs.convertPVCBackupToCSI(readyPb, sourceVolumeID)
	if err != nil {
		return nil, err
	}
...
// generateHostPVCBackupManifest generates a PVCBackup manifest specification
func (cs *ControllerServer) generateHostPVCBackupManifest(name, sourceVolumeID string) (*harvesterv1beta1.PVCBackup, error) {
	labels := map[string]string{
		labelBackupSrcVol: sourceVolumeID,
	}

	pvcBackup := &harvesterv1beta1.PVCBackup{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: cs.namespace,
			Labels:    labels,
		},
		Spec: harvesterv1beta1.PVCBackupSpec{
			Type:   LHBackup,
			Source: sourceVolumeID,
		},
	}

	return pvcBackup, nil
}

func (cs *ControllerServer) generateHostPVCBackup(
	ctx context.Context,
	name, sourceVolumeID string,
) (*harvesterv1beta1.PVCBackup, error) {
	hostPb, err := cs.generateHostPVCBackupManifest(name, sourceVolumeID)
	if err != nil {
		return nil, err
	}

	return cs.createHostPVCBackup(ctx, hostPb)
}

func (cs *ControllerServer) createHostPVCBackupFromReq(
	ctx context.Context,
	name, sourceVolumeID string,
) (*csi.CreateSnapshotResponse, error) {
	logrus.Infof("Creating PVCBackup %s from source volume %s", name, sourceVolumeID)
	createdPb, err := cs.generateHostPVCBackup(ctx, name, sourceVolumeID)
	if err != nil {
		return nil, err
	}

	// Prepare and return response
	response, err := cs.prepareBackupSnapResponse(ctx, createdPb.Name, sourceVolumeID)
	if err != nil {
		return nil, err
	}

	logrus.Infof("Successfully created and prepared PVCBackup %s (size: %d bytes, ready: %t)",
...
func (cs *ControllerServer) handleExistingBackupSnap(
	...
	existingPb, err := cs.getHostPVCBackup(ctx, cs.namespace, name)
	if err != nil {
		return nil, err
	}

	logrus.Infof("PVCBackup %s already exists, returning existing snapshot", existingPb.Name)
	return cs.prepareBackupSnapResponse(ctx, existingPb.Name, sourceVolumeID)
}
...
func (cs *ControllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if isBackupSnapshot {
		// Path 1: Backup snapshot using PVCBackup CR
...
func (cs *ControllerServer) handleBackupSnap(
	...
	return cs.createHostPVCBackupFromReq(ctx, name, sourceVolumeID)
}
...
func (cs *ControllerServer) handleBackupSnapDeletion(
	...
	err := cs.deleteHostPVCBackup(ctx, namespace, name)
	if errors.IsNotFound(err) {
		return &csi.DeleteSnapshotResponse{}, nil
	}
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to delete PVCBackup %s/%s: %v", namespace, name, err)
	}
	return &csi.DeleteSnapshotResponse{}, nil
}
...
// shouldIncludeBackupSnapshot determines if a PVCBackup should be included based on request filters
func (cs *ControllerServer) shouldIncludeBackupSnapshot(pb *harvesterv1beta1.PVCBackup, req *csi.ListSnapshotsRequest) bool {
	// Check if labels exist
	if pb.Labels == nil {
		logrus.Debugf("Skipping PVCBackup %s: missing labels", pb.Name)
		return false
	}

	// Check for required labels
	sourceVol, hasSourceVol := pb.Labels[labelBackupSrcVol]

	if !hasSourceVol {
		logrus.Debugf("Skipping PVCBackup %s: missing required labels", pb.Name)
		return false
	}
...
// convertPVCBackupToEntry converts a PVCBackup to a CSI ListSnapshotsResponse_Entry
func (cs *ControllerServer) convertPVCBackupToEntry(pb *harvesterv1beta1.PVCBackup) (*csi.ListSnapshotsResponse_Entry, error) {
	// Get source volume ID from labels
	sourceVol := ""
	if pb.Labels != nil {
		sourceVol = pb.Labels[labelBackupSrcVol]
	}

	// Convert to CSI snapshot
	csiSnapshot, err := cs.convertPVCBackupToCSI(pb, sourceVol)
	if err != nil {
		return nil, fmt.Errorf("failed to convert PVCBackup %s to CSI format: %w", pb.Name, err)
	}
...
// processHostPVCBackups converts PVCBackups to CSI snapshots with filtering
func (cs *ControllerServer) processHostPVCBackups(
	ctx context.Context,
	req *csi.ListSnapshotsRequest,
	...
	if req.GetSourceVolumeId() != "" {
		logrus.Infof("Filtering PVCBackups by source volume ID: %s", req.GetSourceVolumeId())
		listOptions.LabelSelector = fmt.Sprintf("%s=%s", labelBackupSrcVol, req.GetSourceVolumeId())
	}

	pbList, err := cs.listHostPVCBackups(ctx, "", listOptions)
	if err != nil {
		return nil, err
	}

	csiSnapshots := make([]*csi.ListSnapshotsResponse_Entry, 0, len(pbList.Items))

	for _, pb := range pbList.Items {
		if !cs.shouldIncludeBackupSnapshot(&pb, req) {
			continue
		}

		entry, err := cs.convertPVCBackupToEntry(&pb)
		if err != nil {
			logrus.Warnf("Failed to convert PVCBackup %s: %v", pb.Name, err)
			continue
		}
...
func (cs *ControllerServer) listBackupSnaps(
	...
	entries, err := cs.processHostPVCBackups(ctx, req)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to list host PVCBackups: %v", err)
	}
...
func (cs *ControllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	// List host PVCBackups (backup snapshots)
	backupSnaps, err := cs.listBackupSnaps(ctx, req)
	if err != nil {
		logrus.Warnf("Failed to list host PVCBackups: %v", err)
		backupSnaps = []*csi.ListSnapshotsResponse_Entry{}
	}

	// Combine and return
	allSnaps := append(localSnaps, backupSnaps...)
	logrus.Infof("ListSnapshots returning %d snapshots (%d VolumeSnapshots + %d PVCBackups)",
